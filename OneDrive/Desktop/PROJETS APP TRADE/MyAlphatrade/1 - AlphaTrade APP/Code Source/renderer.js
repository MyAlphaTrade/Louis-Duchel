...
const defaults = {
  ...
  session_target: 25,
  daily_target: 50,
  session_max_loss: -150,
  giveback: 100,
  profit_protection_enabled: true,
  profit_protection_start: 25,
  ...
  reinforcement_min_confidence_margin: 10,
  reinforcement_min_score_gap: 15,
  ...
  symbols: {
    XAUUSD: { lot: .01, lot_min: .001, lot_max: .01, tp_pips: 10, max_positions: 3, max_position_loss: 5, max_floating_loss: 10, timeframe: 'M5', confidence_min: 62, cadence_sec: 30, max_trades_hour: 120, max_hold_sec: 45, position_review_sec: 120, profit_target: .5, profit_lock_trigger: .3, profit_lock_drawdown: .12, emergency_loss_limit: 3, min_positive_exit: .05, signal_reversal_margin: 7, cooldown_after_loss_sec: 75, session_filter_enabled: true, session_start_utc: 8, session_end_utc: 17, stop_before_end_min: 30 },
    EURUSD: { lot: .01, lot_min: .001, lot_max: .02, tp_pips: 8, max_positions: 2, max_position_loss: 3, max_floating_loss: 6, timeframe: 'M15', confidence_min: 68, cadence_sec: 45, max_trades_hour: 40, max_hold_sec: 300, position_review_sec: 120, profit_target: .5, profit_lock_trigger: .3, profit_lock_drawdown: .12, emergency_loss_limit: 4, min_positive_exit: .05, signal_reversal_margin: 9, cooldown_after_loss_sec: 120, session_filter_enabled: true, session_start_utc: 7, session_end_utc: 20, stop_before_end_min: 30 }
  }
};
...
function renderTradingView(force = false) {
  ...
  const symbol = encodeURIComponent(activeSymbol === 'EURUSD' ? 'OANDA:EURUSD' : 'OANDA:XAUUSD');
...
$('tradeBtn').addEventListener('click', () => {
  ...
  setTimeout(() => {
    if (button.classList.contains('busy')) {
      ...
      button.textContent = currentLanguage === 'en' ? 'Stop' : 'Arrêter';
    }
  }, 5000);
});
...
function renderMicrostructurePage() {
  ...
  set('microPagePoc', snapshot.poc ? Number(snapshot.poc).toFixed(activeSymbol === 'EURUSD' ? 5 : 2) : '-');
...
function renderActiveMarket() {
  ...
  $('quantSignal').textContent = `${analysis.quant_signal || 'WAIT'} ${analysis.quant_confidence != null ? `${analysis.quant_confidence}%` : ''}`;
  $('quantSignal').className = String(analysis.quant_signal || 'WAIT').toLowerCase();
  $('quantReason').textContent = analysis.quant_reason || 'Collecte des données';
  ...
  $('microPoc').textContent = snapshot.poc ? Number(snapshot.poc).toFixed(activeSymbol === 'EURUSD' ? 5 : 2) : '-';
}
...
function renderQuality(st) {
  ...
  const bot = currentStatus?.origin_stats?.BOT || {};
...
function renderLearning() {
  ...
  const learnedVix = currentStatus?.learning?.symbols?.EURUSD || {};
  const analysisXau = currentStatus?.analysis?.XAUUSD || {};
  const analysisVix = currentStatus?.analysis?.EURUSD || {};
  ...
  set('iaVixThreshold', `${Number(analysisVix.learned_threshold || params?.symbols?.EURUSD?.confidence_min || 68).toFixed(1)}%`);
  ...
  renderOriginEvaluation('bot', currentStatus?.origin_stats?.BOT || {});
...
function renderServerLearning() {
  ...
  [['XAUUSD', 'serverXau'], ['EURUSD', 'serverVix']].forEach(([symbol, prefix]) => {
...
function calendarStats(key) {
  ...
  const trades = allTrades.filter(t => tradeDay(t) === key && tradeInSessionFilter(t, session));
  ...
  return { trades, total, wins, losses };
}
...
function filteredTrades() {
  ...
  return allTrades.filter(t => {
    ...
    return Number.isFinite(timestamp) && now - timestamp <= windows[tradeFilter];
  });
}
...
function fillSettings(values) {
  ...
  params = {
    ...
    symbols: {
      ...
      EURUSD: { ...defaults.symbols.EURUSD, ...(source.symbols?.EURUSD || {}) }
    }
  };
...
async function doLogin() {
  ...
  try {
    ...
    await applyPlanLimits(data.token, data.plan);
  } catch (e) {
...
async function applyPlanLimits(token, plan) {
  console.log('[Plan] applyPlanLimits appelé, plan recu:', JSON.stringify(plan));
  try {
    const res = await fetch(`${API_BASE}/client/plan-params`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (!res.ok) { console.warn('[Plan] /client/plan-params a retourné', res.status); return; }
    const data = await res.json();
    const planKey   = data.plan_key || 'starter';
    const rawParams = data.params   || {};

    console.log(`[Plan] ${planKey} — ${Object.keys(rawParams).length} params reçus`);

    const savedParams = await alpha.loadParams();
    if (!savedParams) { console.warn('[Plan] loadParams retourne null'); return; }
    if (!savedParams.symbols) savedParams.symbols = {};

    // Appliquer chaque param verrouillé au symbole correspondant
    // Format des clés : ASSET_param_key  (ex. XAUUSD_lot_max, BOOM1000_confidence_min)
    for (const [rawKey, p] of Object.entries(rawParams)) {
      if (!p.is_locked) continue; // non-verrouillé = client premium peut garder sa valeur
      const sep = rawKey.indexOf('_');
      if (sep < 0) continue;
      const asset = rawKey.slice(0, sep);
      const key   = rawKey.slice(sep + 1);
      if (!savedParams.symbols[asset]) continue;
      const raw = String(p.val ?? '');
      let converted;
      if (raw === 'true' || raw === 'false') {
        ...
        converted = raw; // string (ex. timeframe "M5")
      }
      savedParams.symbols[asset][key] = converted;
    }

    // Cap global lot_max (compatibilité arrière)
    const lotMaxGlobal = data.lot_max != null ? parseFloat(data.lot_max) : null;
    if (lotMaxGlobal != null) {
      for (const sym of Object.keys(savedParams.symbols)) {
        if (parseFloat(savedParams.symbols[sym].lot_max || 9999) > lotMaxGlobal) {
          savedParams.symbols[sym].lot_max = lotMaxGlobal;
          savedParams.symbols[sym].lot = Math.min(parseFloat(savedParams.symbols[sym].lot || 0.01), lotMaxGlobal);
        }
      }
    }
    if (data.rebond_enabled != null) savedParams.rebond_enabled = Boolean(data.rebond_enabled);

    await alpha.saveParams(savedParams);
    fillSettings(savedParams);
    sessionStorage.setItem('at_plan_limits', JSON.stringify({ plan_key: planKey, lot_max: lotMaxGlobal, rebond_enabled: data.rebond_enabled, max_positions: data.max_positions }));
    console.log('[Plan] Limites appliquées avec succes');
  } catch (e) {
    console.warn('[Plan] Erreur dans applyPlanLimits:', e);
  }
}
...
function showPlanBadge(user, plan) {
  ...
  badge.id = 'planBadge';
  badge.style.cssText = 'position:fixed;bottom:16px;right:16px;background:#0d1526;border:1px solid #1e2e44;border-radius:10px;padding:10px 16px;font-size:12px;color:#a0b0c8;z-index:999;';
  badge.innerHTML = `<span style="color:#00c896;font-weight:700">${plan.name || plan.plan_key}</span> · ${user.email}<br><span style="color:#4a6080">Expire le ${expires}</span>`;
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.valid) {
          ...
          await applyPlanLimits(token, data.plan);
          return;
        }
      }
    } catch (_) {}
